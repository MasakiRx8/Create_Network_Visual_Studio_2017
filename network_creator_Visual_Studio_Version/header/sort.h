...
public:
	...
	template<typename _T>
	std::vector<_T> ascendingsort_number(std::vector<_T> A)
	{
		int left = 0;
		int right = A.size() - 1;
	...
	template<typename _T>
	std::vector<_T> descendingsort_number(std::vector<_T> A)
	{
		int left = 0;
		int right = A.size() - 1;
		std::vector<int> B = _Create_number(A.size());
...
private:
	...
	template<typename _T>
	void ascendingsort(std::vector<_T>* A, std::vector<_T>* B, int left, int right)
	{
		if (left < right) {
			_T pivot = med3((*A)[left], (*A)[left + (right - left) / 2], (*A)[right]);
			int l = left;
	...
	template<typename _T>
	void descendingsort(std::vector<_T>* A, std::vector<_T>* B, int left, int right)
	{
		if (left < right) {
			_T pivot = med3((*A)[left], (*A)[left + (right - left) / 2], (*A)[right]);
			int l = left;
...
private:
	...
	template<typename _T>
	void ascendingsort(std::vector<_T>* A, std::vector<_T>* B, int n)
	{
		if (n < (*A).size()) {
			int c = 0;
	...
	template<typename _T>
	void descendingsort(std::vector<_T>* A, std::vector<_T>* B, int n)
	{
		if (n < (*A).size()) {
			int c = 0;
			std::vector<_T> ba((*A).size());
